...
2. **Memory management and low-level runtime core — highest priority.**
   ...
   - `crates/monty/src/heap/heap_entries.rs` (521 lines; 17 `unsafe` code blocks, `PAGE_SIZE = 256`).
   - `crates/monty/src/heap_traits.rs` (369 lines; 4 `unsafe` code blocks; `defer_drop!` family).
...
- **Category:** Public-API developer misuse / supply chain (Category 10)
- **Citations:** Same as SA-F-002: `Cargo.lock:1020` and `Cargo.lock:1031`
...
|---|---|---|---|
| 1 | Filesystem access and path traversal | Clean (all paths routed through the 9-step pipeline; no path-traversal finding) | `crates/monty/src/fs/path_security.rs:1-447`, `crates/monty/src/fs/mount_table.rs:1-317`, `crates/monty/src/fs/dispatch.rs:1-231`, `crates/monty/src/fs/error.rs:1-200`, `crates/monty/src/fs/direct.rs` (host-backed backend funnels through `resolve_path`), `crates/monty/src/fs/overlay.rs` (overlay backend also funnels through `resolve_path`) |
| 2 | Low-level code safety | Clean (SA-F-011, SA-F-013 are positive findings; all 53 `unsafe` blocks documented with `SAFETY:` comments) | `crates/monty/src/heap.rs:1-1667`, `crates/monty/src/heap/heap_entries.rs:1-521`, `crates/monty/src/heap_traits.rs:1-369`, Section 5 inventory; Clippy `undocumented_unsafe_blocks = "warn"` lint enforcement |
...
| I-05 | Error messages never contain host paths. | `crates/monty/src/fs/error.rs:1-200` (`MountError::PathEscape` intentionally omits the host path; `into_exception` renders only virtual paths; `stat_result` zeros `uid`/`gid`/`ino`/`dev`) | `crates/monty/tests/fs_security.rs` (error-rendering cases) | **Verified** |
| I-06 | Errno codes are hardcoded POSIX regardless of host OS. | `crates/monty/src/fs/error.rs:1-200` (POSIX errno constants are compiled into `MountError::Io` unconditionally; no platform branch exists) | `crates/monty/tests/fs_security.rs` (errno stability cases) | **Verified** |
